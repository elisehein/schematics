...
describe("CaptionTyping", () => {
  ...
  describe("when constructing a list of typing delay changes from the flags", () => {
    test("resets the typing delay to the previous value after a pause", () => {
      ...
      expect(sut.typingDelayChanges).toEqual(expectedDelayChanges);
    });
    ...
    test("uses a default delay of 0 after pause if no pacing information is given", () => {
      ...
      sut = new CaptionTyping("[PAUSE:SHORT]No typing speed flags.")
      expectedDelayChanges = [
        { index: 0, delay: 300 },
        { index: 1, delay: 0 }
      ];
      expect(sut.typingDelayChanges).toEqual(expectedDelayChanges);
    ...
    test("doesn't add any delay changes if no flags are set", () => {
      const sut = new CaptionTyping("No flags at all.")
      expect(sut.typingDelayChanges).toEqual([]);
    });
  });
...
[PAUSE:LONG]b[TYPE:FAST]ut feel had I stayed,
